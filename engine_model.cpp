...
namespace engine {
    ...
    void Model::bind(VkCommandBuffer commandBuffer){
        ...
        if(hasIndexBuffer){
            vkCmdBindIndexBuffer(commandBuffer, indexBuffer, 0, VK_INDEX_TYPE_UINT32); //We go directly of 32bits so we cna store most models
        }
    }

    //Descriviamo come interpretare il Vertex buffer lato Vk
    ...
    std::vector<VkVertexInputAttributeDescription> Model::Vertex::getAttributeDescriptions(){
        ...
        attributeDescriptions[0].offset = offsetof(Vertex, position); //the order doesn't matter this way.
        ...
        for (const auto &shape : shapes) {
            for(const auto &index : shape.mesh.indices) {
                ...
                if(uniqueVertices.count(vertex) == 0){
                    uniqueVertices[vertex] = static_cast<uint32_t>(vertices.size()); //it will change in size everytime we pushback another vertex
                    vertices.push_back(vertex);
                }
